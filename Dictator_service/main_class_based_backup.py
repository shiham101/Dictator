...
class NmapScan:
	...
	def db_projectname(self,projectname_db,IP_range,Port_range,switch='-T4 -A -n',profile=2): #  Store the project name and return the auto generated id
			"""
			Objective :
			...
			print "Hello"
			print "Obtained Profile id at db_projectname : " +str(profile)
			time.sleep(10)
			try :
				pid=self.IPtable.Insert(projectname_db,IP_range,Port_range,switch,profile)
		...
		try:
			...
			while 1:
				...
				#<<<<<<< HEAD
					...
					print "THreads left are not only dummy :"+str(active_threads)
	...
	def main(self,path='',targethosts='',targetports='',switch='',scan_type='',mode="c",project_id='',assessment_id='',app_id='',concurrent=False,profile=2):
		"""
		Objective :
		...
		if (scan_type=="1"):
			...
			print "The mode recieved is :" +str(mode)+ " and profile at main : "+str(profile)
			if(mode=="c"):
	...
	def driver_main(self,ips='',project_name='',port='',scan_type='',switch='',project_id='',mode="c",assessment_id="",app_id="",concurrent=False,profile=2):
		"""
		Objective :
		...
		try:
			...
			self.main(project_name,ips,port,switch,scan_type,mode,project_id,assessment_id,app_id,concurrent,profile)
